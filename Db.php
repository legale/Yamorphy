 ...
 * ?n ("name")    - identifiers (db, table and field names)
...
class Simpledb
{
    ...
    protected $defaults = array(
        ...
        'pconnect' => FALSE,
        'charset' => 'utf8',
        'errmode' => 'error', //'exception' or 'error'
    ...
    /**
     * This will execute query without throwing an exception or an error.
     * Useful to check if db or table exists.
     *
     * @return mysqli
     */
    public function query_silent()
    {
        $this->silent = true;
        $this->prepared_query = $this->prepareQuery(func_get_args());
        return $this->rawQuery($this->prepared_query);
    }
    ...
    {
        if (!mysqli_ping($this->conn) && !$this->connect($this->opt)) {
            $this->error('mysql reconnect failed');
            return false;
        }
